...
def example_manual_alignment():
    ...
    try:
        ...
        from utils.marker_touch_controller import MarkerTouchController
        controller = MarkerTouchController(
            robot, device, camera, auto_align, rot_align)
...
def example_inspect_markers():
    ...
    try:
        ...
        for i, marker_id in enumerate(ids):
            ...
            logger.info(
                f"  Centroid: ({info.centroid[0]:.1f}, {info.centroid[1]:.1f})")
            logger.info(f"  Area: {info.area:.1f} px²")
            logger.info(f"  Perimeter: {info.perimeter:.1f} px")
            logger.info(
                f"  Dimensions: {info.width_px:.1f}x{info.height_px:.1f} px")
            logger.info("")

        # Dividir por lado
        marker_infos = [detector.get_marker_info(int(ids[i][0]), corners[i])
                        for i in range(len(ids))]
        left, right = detector.split_markers_by_image_center(
            frame, marker_infos)

        logger.info(
            f"Distribuição: {len(left)} esquerda, {len(right)} direita")
...
if __name__ == "__main__":
    ...
    if example == "basic":
        ...
        print("  targeted - Touch specific markers")
