...
class RotationAlignment:
    ...
    def get_frame_with_markers(self, required_count: int = 4) -> Tuple[Optional[np.ndarray], Optional[List[MarkerInfo]]]:
        ...
        if ids is None or len(ids) < required_count:
            self.logger.warning(
                f"Expected {required_count} markers, got {len(ids) if ids is not None else 0}")
            return frame, None
        ...
        if len(ids) > required_count:
            ...
            marker_infos = self.detector.filter_closest_n_markers(
                frame, marker_infos, required_count)
        else:
            corners = self.detector.refine_corners(frame, corners)
            marker_infos = [self.detector.get_marker_info(int(ids[i][0]), corners[i])
                            for i in range(len(ids))]
    ...
    def calculate_correction_angle(self) -> Optional[float]:
        ...
        left_markers, right_markers = self.split_markers_left_right(
            frame, marker_infos)

        if len(left_markers) != self.MARKERS_PER_SIDE or len(right_markers) != self.MARKERS_PER_SIDE:
            self.logger.error(
                f"Invalid split: left={len(left_markers)}, right={len(right_markers)}")
            return None
    ...
    def apply_rz_correction(self, angle_degrees: float) -> bool:
        if abs(angle_degrees) > self.MAX_ROTATION_STEP:
            angle_degrees = self.MAX_ROTATION_STEP * \
                (1 if angle_degrees > 0 else -1)
        ...
        self.logger.info(
            f"Applied RZ correction: {angle_degrees:.2f} deg (gain applied)")
        return True
    ...
    def run_alignment_loop(self, max_iterations: int = None) -> bool:
        ...
        while iteration < max_iterations:
            ...
            self.logger.info(
                f"Iteration {iteration}: Angle error = {angle_error:.2f} deg")
    ...
    def error_diff_between_single_marker_and_image_center_on_mm(
        ...
        id_found, corners = self.detector.detect_single_marker_by_id(
            frame, id_target)
        if id_found is None or corners is None:
            self.logger.warning(
                f"Marker ID {id_target} not found for error calculation")
            return None
        ...
        self.logger.info(
            "Alignment error: x=%.2f mm, y=%.2f mm",
            error_mm_x,
            error_mm_y,
        )
    ...
    def adjust_robot_to_marker_center(
        ...
        error_x = error_diff[0]  # visual error in X
        error_y = error_diff[1]  # visual error in Y

        # =================================================================
        # FAST CONVERGENCE LOGIC
        # =================================================================
        if first_attempt:
            gain_x = 1.0  # Move 100% of the error
            gain_y = 1.0  # Move 100% of the error
            error_y = (error_y / 4.0) * 3.0
            error_x = (error_x / 4.0) * 3.0
            self.logger.info(
                "FIRST ATTEMPT: Applying 75% gain for fast convergence and 3/4 of the error.")
        else:
            min_step_mm = 0.4
            gain_x = min_step_mm if abs(error_y) <= 2 else 0.10
            gain_y = min_step_mm if abs(error_x) <= 2 else 0.10

        # Note the axis swap (robot X receives camera Y and vice versa)
        ...
        self.logger.info(
            ...
            f"error_x={error_x:.2f} mm, error_y={error_y:.2f} mm"
        )
