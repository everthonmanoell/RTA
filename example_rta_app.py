...
def example_complete_workflow(device_type: str = "flat", verbose: bool = False):
    ...
    try:
        ...
        logger.info(
            f"  Esperado: {controller.app_manager.get_expected_marker_count()} markers")
        logger.info("")
        ...
        logger.info("")
        logger.info(
            f"Resumo: {successful}/{len(markers)} markers tocados com sucesso")
        logger.info("")
        ...
        if successful == len(markers):
            ...
            logger.info(
                f"⚠ PARCIAL: {successful}/{len(markers)} markers tocados")
        else:
...
if __name__ == "__main__":
    ...
    if args.mode == "complete":
        ...
        print("Test: Checking connections...")
        from drivers.device.app_manager import DeviceAppManager
